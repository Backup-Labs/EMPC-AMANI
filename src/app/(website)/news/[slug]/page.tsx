      ...
      <ul className="flex flex-col gap-2">
        {headings.map(({ id, text }) => (
          <li key={id}>
            <a
              href={`#${id}`}
              className={`text-sm font-medium no-underline transition-colors block py-1 border-l-2 pl-3 ${
                activeId === id
...
function ContentBlock({ block, index }: { block: NewsContentBlock; index: number }) {
  if (block.type === "h2") {
    ...
    return (
      <h2 id={id} className="font-black text-[1.75rem] md:text-[2.2rem] tracking-[-0.04em] leading-tight mt-10 mb-4 text-foreground scroll-mt-32">
        {block.text}
      </h2>
    );
  }
  if (block.type === "h3") {
    const id = `heading-${index}`;
    return (
      <h3 id={id} className="font-black text-xl tracking-tight mt-8 mb-3 text-foreground scroll-mt-32">
        {block.text}
      </h3>
    );
  }
  ...
  return (
    <p className="text-foreground/65 text-[1.0625rem] leading-[1.75] mb-5">{block.text}</p>
  );
}
  ...
  const headings = post.content
    .map((block, i) => {
      if (block.type === "h2" || block.type === "h3") {
        return { id: `heading-${i}`, text: block.text };
      }
      return null;
    })
      ...
      <section className="pt-28 md:pt-32 pb-8 px-6 md:px-12 lg:px-16">
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <h1 className="font-black text-[2.2rem] md:text-[3.2rem] lg:text-[3.8rem] leading-[0.92] tracking-[-0.04em] m-0">
              {post.title}
            </h1>
            ...
            <article className="max-w-2xl mx-auto xl:mx-0 prose-article">
              ...
              {post.tags && post.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-10 pt-8 border-t border-border">
                  {post.tags.map((tag) => (
                    <Badge key={tag} className="bg-foreground/5 text-foreground/50">
                      {tag}
                    </Badge>
                  ))}
                </div>
                ...
                <div className="flex gap-2">
                  {[
                    ...
                    <button
                      ...
                      className="h-10 px-4 rounded-full border border-border flex items-center justify-center text-[11px] font-black tracking-widest hover:bg-primary hover:text-background hover:border-primary transition-all"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {related.map((item, i) => (
                <RevealOnScroll key={item.slug} delay={i * 0.08}>
                  <Link href={`/news/${item.slug}`} className="group block no-underline text-foreground">
